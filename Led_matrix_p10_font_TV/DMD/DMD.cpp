...
void DMD::drawString(int bX, int bY,unsigned char *bChars, byte length,
		     byte bGraphicsMode)
{
    if (bX >= (DMD_PIXELS_ACROSS*DisplaysWide) || bY >= DMD_PIXELS_DOWN * DisplaysHigh)
	return;
...
void DMD::drawMarquee(unsigned char *bChars, byte length, int left, int top)
{
    marqueeWidth = 0;
    for (int i = 0; i < length; i++) {
...
int DMD::drawChar(const int bX, const int bY, unsigned char letter, byte bGraphicsMode)
{
...
int DMD::charWidth(unsigned char letter)
{
   unsigned char c = letter;
