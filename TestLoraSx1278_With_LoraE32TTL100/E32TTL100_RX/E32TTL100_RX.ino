...
//SoftwareSerial Serial2(SOFT_RX, SOFT_TX);  // RX, TX
...
RET_STATUS Module_info(uint8_t* pReadbuf, uint8_t buf_len)
{
  ...
  Readcnt = Serial2.available();
  //Serial.print("Serial2.available(): ");  Serial.print(Readcnt);  Serial.println(" bytes.");
  if (Readcnt == buf_len)
  {
    for(idx=0;idx<buf_len;idx++)
    {
      *(pReadbuf+idx) = Serial2.read();
      Serial.print(" 0x");
...
#else
  uint8_t SendBuf[13] = { DEVICE_A_ADDR_H, DEVICE_A_ADDR_L, 0x17, 'H','i',0x32,0x33,0x34,0x35,0x36,0x37, 0x38,0x39};	//for B
#endif
  delay(10);
  Serial2.write(SendBuf, 13);
